...
def test_create_activity_should_succeed_with_valid_request(
    ...
    repository_create_mock = mocker.patch.object(
        activity_dao.repository, 'create', return_value=fake_activity
    )
...
def test_create_activity_should_reject_bad_request(
    client: FlaskClient, mocker: MockFixture, valid_header: dict
):
    from time_tracker_api.activities.activities_namespace import activity_dao
    ...
    response = client.post(
        "/activities", headers=valid_header, json=None, follow_redirects=True
    )
...
def test_list_all_active(
    ...
    repository_find_all_mock = mocker.patch.object(
        activity_dao.repository, 'find_all', return_value=[]
    )
...
def test_get_activity_should_succeed_with_valid_id(
    ...
    repository_find_mock = mocker.patch.object(
        activity_dao.repository, 'find', return_value=fake_activity
    )

    response = client.get(
        "/activities/%s" % valid_id,
        headers=valid_header,
        follow_redirects=True,
    )
...
def test_get_activity_should_return_not_found_with_invalid_id(
    ...
    repository_find_mock = mocker.patch.object(
        activity_dao.repository, 'find', side_effect=NotFound
    )

    response = client.get(
        "/activities/%s" % invalid_id,
        headers=valid_header,
        follow_redirects=True,
    )
...
def test_get_activity_should_return_422_for_invalid_id_format(
    ...
    repository_find_mock = mocker.patch.object(
        activity_dao.repository, 'find', side_effect=UnprocessableEntity
    )
...
def test_update_activity_should_succeed_with_valid_data(
    client: FlaskClient, mocker: MockFixture, valid_header: dict
):
    from time_tracker_api.activities.activities_namespace import activity_dao

    repository_update_mock = mocker.patch.object(
        activity_dao.repository, 'partial_update', return_value=fake_activity
    )
    ...
    repository_update_mock.assert_called_once_with(
        str(valid_id), valid_activity_data, ANY
    )


def test_update_activity_should_reject_bad_request(
    client: FlaskClient, mocker: MockFixture, valid_header: dict
):
    from time_tracker_api.activities.activities_namespace import activity_dao
...
def test_update_activity_should_return_not_found_with_invalid_id(
    client: FlaskClient, mocker: MockFixture, valid_header: dict
):
    from time_tracker_api.activities.activities_namespace import activity_dao
    from werkzeug.exceptions import NotFound
    ...
    repository_update_mock = mocker.patch.object(
        activity_dao.repository, 'partial_update', side_effect=NotFound
    )
    ...
    assert HTTPStatus.NOT_FOUND == response.status_code
    repository_update_mock.assert_called_once_with(
        str(invalid_id), valid_activity_data, ANY
    )


def test_delete_activity_should_succeed_with_valid_id(
    client: FlaskClient, mocker: MockFixture, valid_header: dict
):
    from time_tracker_api.activities.activities_namespace import activity_dao
    ...
    repository_remove_mock = mocker.patch.object(
        activity_dao.repository, 'partial_update', return_value=None
    )
...
def test_delete_activity_should_return_not_found_with_invalid_id(
    client: FlaskClient, mocker: MockFixture, valid_header: dict
):
    from time_tracker_api.activities.activities_namespace import activity_dao
    from werkzeug.exceptions import NotFound
...
def test_delete_activity_should_return_422_for_invalid_id_format(
    client: FlaskClient, mocker: MockFixture, valid_header: dict
):
    from time_tracker_api.activities.activities_namespace import activity_dao
    from werkzeug.exceptions import UnprocessableEntity
