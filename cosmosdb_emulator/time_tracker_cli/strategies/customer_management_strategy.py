...
class CustomerManagementStrategy(ManagementStrategy):

    _conflict_entities: set = {
        TimeTrackerEntities.CUSTOMER.value,
    ...
    def get_confirmation_to_delete_data(self) -> bool:
        is_user_agree_to_delete_customers_data = ask_delete_confirmation(
            self.get_conflict_entities()
        )
        return is_user_agree_to_delete_customers_data
